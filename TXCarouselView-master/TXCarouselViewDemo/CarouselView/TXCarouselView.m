...
{
    NSLog(@"self.modelArray.count==%ld",(long)self.modelArray.count);
...
-(void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    if (scrollView.contentOffset.x<300) {
        CGPoint point = CGPointMake(0+(self.modelArray.count/2*self.frame.size.height*itemHight),self.collectionView.contentOffset.y);
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    NSLog(@"indexPath.row==%ld",(long)indexPath.row%5);
